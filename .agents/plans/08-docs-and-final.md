...
- Confirm the EAR packages `META-INF/jboss-deployment-structure.xml` with `org.slf4j` dependencies
  for the deployment and the `jboss-log-viewer-api.war` sub-deployment.
